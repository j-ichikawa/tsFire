javascript:(async function () {
    ...
    const weekly_report_endTime = dt.getHours().toString() + ':' + dt.getMinutes().toString();
    ...
    for (let i = startDate; i <= endDate; i++) {
        ...
        if (time) {
            ...
            if (isFriday) {
                document.getElementById('endTime').value = weekly_report_endTime;
                document.getElementById('startOut1').value = endTime;
                document.getElementById('endOut1').value = weekly_report_endTime;
            } else {
